...
- **Config views go stale.** Config writes are scoped and atomic — each save merges only its own entry into a freshly read file — so concurrent instances no longer discard each other's changes on disk. But every process still holds its own in-memory copy: entries another instance added or changed stay invisible to it until restart.
